...
deploy script sets `NEXT_PUBLIC_BASE_PATH`, which drives `basePath`/`assetPrefix` and
the `appUrl()` helper in `lib/basePath.ts` that share links are built with. To host
at a domain root instead, build without that variable and publish `out/`.
